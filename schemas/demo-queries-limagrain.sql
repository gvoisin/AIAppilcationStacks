-- Requetes Oracle de demonstration adaptees aux enregistrements LIMAGRAIN Vegetable Seeds.
-- Ordre de chargement :
--   1. @schemas/demo-schema.sql
--   2. @schemas/demo-data-limagrain.sql
--   3. Executer les requetes ci-dessous selon le besoin
...
--------------------------------------------------------------------------------
-- 2) Resume les impacts sur la filiere de processing et qualite des semences
--    potageres en Europe du Sud.
--------------------------------------------------------------------------------
SELECT
    f.filiere_name,
    t.territory_name,
...
WHERE f.filiere_code = 'VEG_PROCESSING'
  AND t.territory_code = 'SOUTH_EU'
GROUP BY
    f.filiere_name,
    t.territory_name,
    oa.alert_code,
...
--------------------------------------------------------------------------------
-- 3) Quels territoires menacent HM.CLAUSE ou Hazera ?
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    t.territory_name,
...
--------------------------------------------------------------------------------
-- 4) Compare la performance logistique entre la production et la distribution
--    commerciale des semences potageres.
--------------------------------------------------------------------------------
SELECT
    f.filiere_name,
...
--------------------------------------------------------------------------------
-- 6) Donne moi une synthese des risques pouvant affecter Vilmorin-Mikado.
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    oa.alert_code,
...
--------------------------------------------------------------------------------
-- 7) Quels incidents pourraient impacter les livraisons HM.CLAUSE cette semaine ?
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    oa.alert_code,
...
--------------------------------------------------------------------------------
-- 8) Y a t il un risque pour les expeditions Vilmorin-Mikado vers l APAC ?
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    l.lot_code,
...
--------------------------------------------------------------------------------
-- 9) Quels sujets doivent etre remontes a un responsable qualite pour HM.CLAUSE ?
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    qi.issue_code,
...
--------------------------------------------------------------------------------
-- 10) Fais une synthese executive en 5 points pour un directeur de la ligne de
--     metier semences potageres LIMAGRAIN.
--------------------------------------------------------------------------------
SELECT summary_line
FROM (
...
--------------------------------------------------------------------------------
-- 11) Construis un espace de pilotage pour un risque qualite sur les semences
--     potageres avec impacts potentiels sur HM.CLAUSE et Limagrain Vegetable Seeds.
--------------------------------------------------------------------------------
SELECT
    oa.alert_code,
    oa.alert_title,
...
WHERE f.filiere_code = 'VEG_PROCESSING'
  AND b.brand_name IN ('HM.CLAUSE', 'Limagrain Vegetable Seeds')
ORDER BY
    CASE oa.severity
        WHEN 'CRITICAL' THEN 1
        WHEN 'HIGH' THEN 2
...
--------------------------------------------------------------------------------
-- 12) Prepare un cockpit de suivi des expeditions export pour Vilmorin-Mikado
--     avec alertes de tracabilite, lots a risque et priorites de traitement.
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    lf.flow_name,
...
--------------------------------------------------------------------------------
-- 13) Cree une vue de pilotage des filieres sous marque Limagrain Vegetable Seeds
...
LEFT JOIN alert_brand_impacts abi ON abi.brand_id = b.brand_id
LEFT JOIN operational_alerts oa ON oa.alert_id = abi.alert_id
WHERE b.brand_name = 'Limagrain Vegetable Seeds'
...
--     doit suivre Hazera, Vilmorin-Mikado et Limagrain Vegetable Seeds en APAC.
--------------------------------------------------------------------------------
SELECT
    b.brand_name,
    oa.alert_title,
...
WHERE b.brand_name IN ('Hazera', 'Vilmorin-Mikado', 'Limagrain Vegetable Seeds')
ORDER BY
    b.brand_name,
    CASE ap.priority
        WHEN 'CRITICAL' THEN 1
